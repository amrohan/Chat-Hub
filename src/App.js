...
function App() {
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex items-center justify-center shadow-md h-full truncate">
        <div className="flex flex-col items-center justify-center max-w-xl w-full mx-4 p-8 rounded-md shadow-card bg-white dark:bg-coolDark-600 transition-all">
          ...
          <p className="mb-8 text-lg text-center">
            To join the chat room, you must first sign in.
          </p>
          <button
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-950 dark:text-white transition-colors">
      <header
        className="flex-shrink-0 w-full flex items-center justify-evenly  px-6 sm:px-8 shadow-md"
        style={{ height: "var(--topbar-height)" }}
      >
        <a href="https://rohan.ml">
          <svg
            width="24"
            height="24"
            xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="flex w-full text-right content-end">
          {user ? (
            <button
              onClick={signOut}
              className="uppercase w-full text-center text-sm font-medium text-primary-500 hover:text-white tracking-wide hover:bg-primary-500 bg-transparent rounded py-2 px-4 mr-4 focus:outline-none focus:ring focus:ring-primary-500 focus:ring-opacity-75 transition-all"
            >
              Sign out
            </button>
          ) : null}
